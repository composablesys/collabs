...
export function showhidehelp()
{
	if (document.getElementById('showhidehelp').innerHTML == "show help")
		...
		document.getElementById('showhidehelp').innerHTML = "show help";
}
...
export function reverseAllele(allele)
{
	...
	debugReader.innerHTML+='for allele '+thisAllele+' turning '+thisType+' to '+newType+'<br>returning path "'+path + newType+'.png';+'"';
...
export function evaluateGenetics()
{
	...
	//relgular white
		...
		//rabicano
		rabicano = (((alleles['rb1'].value === "rb")) || (alleles['rb2'].value === "rb")) ? 1:0;
		//sooty?
		sooty = ((alleles['sty1'].value === "sty") || (alleles['sty2'].value === "sty")) ? 1:0;
		...
		if ((alleles['cr1'].value === "cr") && (alleles['cr2'].value === "cr")) {
			...
			spotSize-=2;
		//double pearl
		} else if ((alleles['cr1'].value === "prl") && (alleles['cr2'].value === "prl")) {
			if (color == "wild_bay") color = "pearl_bay";
			else color = "pearl_"+color;
			spotSize-=2;
		//cream pearl
		} else if (((alleles['cr1'].value === "prl") && (alleles['cr2'].value === "cr"))
		...
		//paint?
		tobiano = ((alleles['to1'].value === "to") || (alleles['to2'].value === "to")) ? 1:0;
		...
		blanket = ((alleles['patn21'].value === "patn2") || (alleles['patn22'].value === "patn2")) ? 1 : 0;
		...
		if ((varnish==1) && (leopard || blanket)) {
			self.document.images['roan'].src = hpath + 'roan_appaloosa'+spotSize+'.png';
			self.document.images['rabicano'].src = hpath + 'rabicano_appaloosa'+spotSize+'.png';
		} else {
			self.document.images['roan'].src = hpath + 'roan.png';
			self.document.images['rabicano'].src = hpath + 'rabicano.png';
		}
		roan = ((alleles['rn1'].value === "rn") || (alleles['rn2'].value === "rn")) ? 1:0;
	}

	//only reload images if something's changed
	if ((oldColor != color) || (oldGrey != grey) || (oldRoan != roan) || (oldRabicano != rabicano) || (oldSooty != sooty) || (oldTobiano != tobiano) || (oldOvero != overo) || (oldSplashed != splashed) || (oldVarnish != varnish) || (oldLeopard != leopard) || (oldBlanket != blanket) || (oldSabino != sabino)) {
...
export function updateDisplay(newColor, isGrey, isRoan, isRabicano, isSooty, isTobiano, isOvero, isSplashed, isVarnish, isLeopard, isBlanket, isSabino)
{
	if (newColor != 'lwa' && newColor != 'lwo') {
		...
		document.getElementById('rabicano').style.display = isRabicano? "block":"none";
		document.getElementById('sooty').style.display = isSooty? "block":"none";
		document.getElementById('tobiano').style.display = isTobiano? "block":"none";
		document.getElementById('overo').style.display = isOvero? "block":"none";
		document.getElementById('splashedwhite').src = hpath + "splash"+isSplashed+".png";
		document.getElementById('splashedwhite').style.display = isSplashed? "block":"none";
		document.getElementById('sabino').src = hpath + "sabino"+isSabino+".png";
		document.getElementById('sabino').style.display = isSabino? "block":"none";
		document.getElementById('white').style.display = (newColor == 'white') ? "block":"none";
		...
		document.getElementById('appaloosa').style.display = isVarnish? "block":"none";
...
//	self.document.body.style.cursor = "wait";
	...
	tempImage.onload=function(){isLoaded()};
	var tempSrc;
...
export function updateName(newColor)
{
	...
	case 'silver_champagne_classic':
		document.getElementById('colorname').innerHTML = "Silver Classic Champagne";
		break;
	...
	case 'flaxen_gold_pearl':
		document.getElementById('colorname').innerHTML = "Flaxen Apricot Champagne";
		break;
	case 'silver_black':
	...
	case 'silver_sealbrown_cream2':
		document.getElementById('colorname').innerHTML =  "Silver Seal Brown Cream";
		break;
	...
	case 'silver_dun_bay_cream2':
		document.getElementById('colorname').innerHTML = "Silver Dapple Perlino Dun";
		break;
	case 'silver_dun':
		document.getElementById('colorname').innerHTML = "Silver Dapple Dun";
		break;
	case 'silver_dun_black_cream2':
		document.getElementById('colorname').innerHTML = "Silver Dapple Cream Grulla";
		break;
...
export function updateDescription(newColor)
{
	...
	{
	case 'lwa':
		document.getElementById('colorinfo').children[0].innerHTML = "Dominant white homozygosity is lethal, and a WW fetus will be naturally aborted by the mare early in gestation.<br><br>This differs from Overo Lethal White Syndrome in which outwardly healthy appearing foals will be born only to die within their first days.";
		break;
	case 'lwo':
		document.getElementById('colorinfo').children[0].innerHTML = "Also called Overo Lethal White Syndrome (OLWS) or Lethal White Overo (LWO).<br><br>The frame overo gene is linked to a mutation which in its homozygous state will cause a malformed colon.<br><br>Such foals will die painfully within a few days of birth, and are usually humanely euthanized as soon as they are identified.<br><br>This differs from the lethality of homozygous dominant whites, in which the fetus is naturally aborted by the mare early in gestation.";
		break;
	case 'white':
		document.getElementById('colorinfo').children[0].innerHTML = "A truly white horse has pink skin and white hair, with blue or light-colored eyes.  Unlike greys that appear white, a true white's coat will be white from birth.<br><br>Dominant white is an allele of the KIT gene, making white mutually exclusive from homozygous tobiano, roan, and sabino.";
		break;
	case 'grey':
		document.getElementById('colorinfo').children[0].innerHTML = "A grey horse will be born looking like any ordinary colored horse.  Gradually over time, more and more white hairs will appear in their coat until eventually they are stark white.  How quickly the greying occurs varys dramatically from horse to horse.  Some will show signs at 3 to 4 months, while others show no sign until several years of age.<br><br>A grey horse that was born a dark color can still be indentified after it has faded to pure white by its underlying skin color.  This is especially true of horses with unique skin patterns such as appaloosa or champagne.<br><br>Lipizzaner stallions have made this grey coloration famous.";
		break;
	case 'chestnut':
		document.getElementById('colorinfo').children[0].innerHTML = "Chestnut is one of the most common horse coat colors, seen in almost every breed.  Chestnut consists of a red or brownish coat, with a mane and tail the same or lighter in color than the coat.";
		break;
	case 'black':
		document.getElementById('colorinfo').children[0].innerHTML = "Black is a relatively uncommon coat color in horses, though not so unusual as to be considered rare. For a horse to be considered black, it must be completely black except for white markings.<br><br>A visible difference between a true black and a dark chestnut or bay is seen in the fine hairs around the eyes and muzzle; on a true black these hairs remain black even if the horse is sun-bleached, while on other colors they will be lighter.";
		break;
	case 'bay':
		document.getElementById('colorinfo').children[0].innerHTML = "Bay coloring is a very common coat color in horses.  It is characterized by a reddish brown body with black &quot;points.&quot;  These points include the mane, tail, lower legs, and sometimes the tips of the ears.  Bay horses have black skin under their coat, except beneath white markings.";
		break;
	case 'wild_bay':
		document.getElementById('colorinfo').children[0].innerHTML = "Bay coloring is a very common coat color in horses.  It is characterized by a reddish brown body with black &quot;points.&quot;  One alternative form of the Agouti gene will result in a wild type bay with a lighter body and very little black on its legs.";
		break;
	case 'sealbrown':
		document.getElementById('colorinfo').children[0].innerHTML = "Also called &quot;black and tan&quot; in some countries, a seal brown horse has an alternative form of the Agouti gene which restricts its black pigment from the soft areas of its body.";
		break;
	case 'chestnut_cream1':
		document.getElementById('colorinfo').children[0].innerHTML = "A chestnut with a single cream gene, the palomino is a golden colored horse with nearly white mane and tail.  The lightest palominos can be described as ivory colored, and many palominos are lighter in winter than in summer.<br><br>An ivory palomino can be distinguished from a cremello by the color of its skin.";
		break;
	case 'black_cream1':
		document.getElementById('colorinfo').children[0].innerHTML = "When a black horse has a cream gene, its color gets diluted to a dark brown.  Thus, a smoky black is visibly indistinguishable from a dark chestnut.  However, a smoky black can produce a double-cream foal whereas a chestnut cannot.";
		break;
	case 'bay_cream1':
		document.getElementById('colorinfo').children[0].innerHTML = "A single cream gene acting on a bay horse affects only the reddish hair of the body, lightening it to gold.  The stark black &quot;points&quot; are left undiluted.  Buckskins are often confused with duns, which have a similar coat color.  However, buckskins lack the &quot;primitive&quot; markings of the dun.";
		break;
	case 'wild_bay_cream1':
		document.getElementById('colorinfo').children[0].innerHTML = "A single cream gene acting on a bay horse affects only the reddish hair of the body, lightening it to gold.  The stark black &quot;points&quot; are left undiluted.<br><br>One alternative form of the Agouti gene will result in a wild type buckskin with very little black on its legs.<br><br>Buckskins are often confused with duns, which have a similar coat color.  However, buckskins lack the &quot;primitive&quot; markings of the dun.";
		break;
	case 'sealbrown_cream1':
		document.getElementById('colorinfo').children[0].innerHTML = "Also called &quot;black and tan&quot; in some countries, a seal brown horse has an alternative form of the Agouti gene which restricts its black pigment from the soft areas of its body.  These red areas can be affected by the cream gene.";
		break;
	case 'chestnut_cream2':
		document.getElementById('colorinfo').children[0].innerHTML = "A chestnut with two cream alleles, the cremello is a cream colored horse with a white mane and tail.  It has pink skin, blue eyes, and often fades to pure white as an adult.<br><br>A cremello can be differentiated from an ivory palomino by the color of its skin.";
		break;
	case 'black_cream2':
		document.getElementById('colorinfo').children[0].innerHTML = "Two cream genes will dilute a black horse's coat to near white.  With pink skin, blue eyes, and often fading to pure white as an adult, a smoky cream can be very difficult to tell apart from a true white, despite the fact that it came from a black horse.";
		break;
	case 'bay_cream2':
		document.getElementById('colorinfo').children[0].innerHTML = "A bay horse with two cream genes, the perlino has a cream colored body and a yellowish or tan cast to its mane and tail.  It has pink skin and blue eyes.";
		break;
	case 'wild_bay_cream2':
		document.getElementById('colorinfo').children[0].innerHTML = "A bay horse with two cream genes, the perlino has a cream colored body and a yellowish or tan cast to its mane and tail.  It has pink skin and blue eyes.<br><br>One alternative form of the Agouti gene will result in a wild type perlino with very little dark on its legs.";
		break;
	case 'sealbrown_cream2':
		document.getElementById('colorinfo').children[0].innerHTML = "Two cream genes will dilute a seal brown horse's coat to near white.  With pink skin, blue eyes, and often fading to pure white as an adult, a smoky cream can be very difficult to tell apart from a true white, despite the fact that it came from a dark horse.";
		break;
	case 'champagne_classic':
		document.getElementById('colorinfo').children[0].innerHTML = "Champagne horses have pink, freckled skin and eyes often described as hazel.  When a black horse receives a champagne gene, its coat is diluted to a light, brownish grey which can appear to have a purplish cast due to the pink skin underneath.";
		break;
	case 'champagne_amber':
		document.getElementById('colorinfo').children[0].innerHTML = "Champagne horses have pink, freckled skin and eyes often described as hazel.  When a bay horse receives a champagne gene, it will look similar to a buckskin with a golden body and darker points.  However, the points are brownish rather than black, and often the legs are not particularly dark.";
		break;
	case 'champagne_gold':
		document.getElementById('colorinfo').children[0].innerHTML = "Champagne horses have pink, freckled skin and eyes often described as hazel.  When a chestnut horse receives a champagne gene, it will look very much like a palomino with a golden body and a mane and tail that are either gold or white.  A gold champagne can be distinguished from a palomino by its pink skin and freckling.";
		break;
	case 'champagne_sable':
		document.getElementById('colorinfo').children[0].innerHTML = "Champagne horses have pink, freckled skin and eyes often described as hazel.  When a seal brown horse receives a champagne gene, it will look like a cross between a classic champagne and an amber champagne.";
		break;
	case 'silver_champagne_classic':
		document.getElementById('colorinfo').children[0].innerHTML = "Champagne horses have pink, freckled skin and eyes often described as hazel.  When a black horse receives a champagne gene, its coat is diluted to a light, brownish grey which can appear to have a purplish cast due to the pink skin underneath.<br><br>The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen.";
		break;
	case 'silver_champagne_amber':
		...
		break;
	case 'silver_champagne_sable':
		document.getElementById('colorinfo').children[0].innerHTML = "Champagne horses have pink, freckled skin and eyes often described as hazel.  When a seal brown horse receives a champagne gene, it will look like a cross between a classic champagne and an amber champagne.<br><br>The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen.";
		break;
	case 'champagne_classic_cream':
		document.getElementById('colorinfo').children[0].innerHTML = "Champagne horses have pink, freckled skin and eyes often described as hazel.  When a black horse receives a champagne gene and a cream gene, it is called a classic cream.";
		break;
	case 'champagne_amber_cream':
		document.getElementById('colorinfo').children[0].innerHTML = "Champagne horses have pink, freckled skin and eyes often described as hazel.  When a bay horse receives a champagne gene and a cream gene, it is called an amber cream.";
		break;
	case 'champagne_gold_cream':
		document.getElementById('colorinfo').children[0].innerHTML = "When a chestnut horse receives a champagne gene and a cream gene, it is either called a gold cream or an ivory champagne.  The term ivory champagne grew popular before the cream gene and champagne genes were understood, and it was believed that this was merely a different kind of champagne horse.";
		break;
	case 'champagne_sable_cream':
		document.getElementById('colorinfo').children[0].innerHTML = "Champagne horses have pink, freckled skin and eyes often described as hazel.  When a seal brown horse receives a champagne gene and a cream gene, it is called a sable cream.";
		break;
	case 'dun_bay':
		document.getElementById('colorinfo').children[0].innerHTML = "Duns are characterized by a lighter body color and &quot;primitive&quot; markings which include leg barring and a dorsal stripe.  Less common primitive markings incude ear barring, spider-webbed forehead, a lower face mask, and light &quot;guard hairs&quot; on the edges of the mane and tail.<br><br>The classic dun is often confused with a buckskin, as both have black points and a light colored body.  However, a buckskin is usually more golden rather than tan, and a buckskin won't have primitive markings.";
		break;
	case 'dun_chestnut':
		...
		break;
	case 'dun_black':
		document.getElementById('colorinfo').children[0].innerHTML = "Also called grullo or blue dun.  Duns are characterized by a lighter body color and &quot;primitive&quot; markings which include leg barring and a dorsal stripe.  Less common primitive markings incude ear barring, spider-webbed forehead, a lower face mask, and light &quot;guard hairs&quot; on the edges of the mane and tail.<br><br>A grullo's coat will not fade with age and does not have white hairs mixed in as a roan does.  Its grey coloring is solid.";
		break;
	case 'dun_sealbrown':
		document.getElementById('colorinfo').children[0].innerHTML = "A seal brown dun can be difficult to differentiate from a bay dun.  Both have dull tan to brown coats with black &quot;primitive markings.&quot;  These primitive markings include leg barring and a dorsal stripe.  Less common primitive markings incude ear barring, spider-webbed forehead, a lower face mask, and light &quot;guard hairs&quot; on the edges of the mane and tail.";
		break;
	case 'dun_chestnut_cream1':
		document.getElementById('colorinfo').children[0].innerHTML = "A chestnut with a single cream gene and a dun gene, the dunalino has the golden coat and white mane and tail of the palomino, but red &quot;primitive markings.&quot;";
		break;
	case 'dun_chestnut_cream2':
		...
		break;
	case 'dun_bay_cream1':
		document.getElementById('colorinfo').children[0].innerHTML = "When a bay horse gets a cream gene and a dun gene, first the cream gene lightens the horse's body to gold.  Then, the dun gene puts black back onto its body in the form of &quot;primitive markings.&guot;  Dunskins thus help to further confuse the differnce between a buckskin and a dun.";
		break;
	case 'dun_bay_cream2':
		document.getElementById('colorinfo').children[0].innerHTML = "A bay horse with two cream genes, the perlino has a cream colored body and a yellowish or tan cast to its mane and tail.  Adding in a dun gene will reveal &quot;primitive markings&quot; in the same yellowish color as the mane and tail.";
		break;
	case 'dun_black_cream1':
		document.getElementById('colorinfo').children[0].innerHTML = "A black horse with a single cream gene will produce a dark chocolate colored horse.  Adding in a dun gene dilutes the body color to a dull tan while darkening the &quot;primitive markings&quot; back to their original black.<br><br>These primitive markings include leg barring and a dorsal stripe.  Less common markings incude ear barring, spider-webbed forehead, a lower face mask, and light &quot;guard hairs&quot; on the edges of the mane and tail.";
		break;
	case 'dun_black_cream2':
		document.getElementById('colorinfo').children[0].innerHTML = "A black horse with two cream genes and a dun gene will look much like a perlino dun, with a cream colored body and a yellowish or tan cast to its mane, tail, and &quot;primitive markings.&quot;";
		break;
	case "dun_champagne_gold":
	case "dun_champagne_sable":
	case "dun_champagne_amber":
	case "dun_champagne_classic":
		document.getElementById('colorinfo').children[0].innerHTML = 'Champagne horses have pink, freckled skin and eyes often described as hazel.<br><br>Duns are characterized by a lighter body color and "primitive" markings which include leg barring and a dorsal stripe. Less common primitive markings incude ear barring, spider-webbed forehead, a lower face mask, and light "guard hairs" on the edges of the mane and tail.<br><br>When a horse is both champagne and dun its body becomes very light, but it retains its dun primitive markings and champagne freckled skin.';
		break;
	case 'flaxen_dun':
	...
	case 'flaxen':
		...
		break;
	case 'silver_black':
		document.getElementById('colorinfo').children[0].innerHTML = "The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen because of the blonde chocolate appearance it can give.";
		break;
	case 'silver_black_cream2':
		...
		break;
	case 'silver_bay':
		document.getElementById('colorinfo').children[0].innerHTML = "The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen.<br><br>A silver dapple bay looks similar to a flaxen chestnut, but has dark brown legs.";
		break;
	case 'silver_wild_bay':
		document.getElementById('colorinfo').children[0].innerHTML = "The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen.<br><br>A silver dapple wild bay looks very similar to a flaxen chestnut, but has dark brown near its hooves.";
		break;
	case 'silver_sealbrown':
	case 'silver_sealbrown_cream1':
		document.getElementById('colorinfo').children[0].innerHTML =  "The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen.<br><br>A silver dapple seal brown horse will have a brown body of various shades with a white mane and tail.";
		break;
	case 'silver_sealbrown_cream2':
		...
		break;
	case 'silver_bay_cream1':
		document.getElementById('colorinfo').children[0].innerHTML = "The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen.<br><br>A silver dapple buckskin looks similar to a palomino, but has dark brown legs.";
		break;
	case 'silver_bay_cream2':
		...
		break;
	case 'silver_wild_bay_cream1':
		document.getElementById('colorinfo').children[0].innerHTML = "The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen.<br><br>A silver dapple wild buckskin looks very similar to a palomino, but has dark brown on its lower legs.";
		break;
	case 'silver_dun_bay_cream1':
		...
		break;
	case 'silver_dun_bay_cream2':
		document.getElementById('colorinfo').children[0].innerHTML = "The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail.<br><br>A bay horse with two cream genes, the perlino has a cream colored body and slightly darker points.  Adding in a dun gene will reveal &quot;primitive markings.&quot;";
		break;
	case 'silver_dun':
		document.getElementById('colorinfo').children[0].innerHTML = "The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen.<br><br>A silver dun looks similar to a silver bay; however, a silver dun will have &quot;primitive&quot; markings.";
		break;
	case 'silver_dun_black_cream2':
		document.getElementById('colorinfo').children[0].innerHTML = "The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen.<br><br>A black horse with two cream genes and a dun gene will look much like a perlino dun, with a cream colored body.";
		break;
	case 'silver_dun_sealbrown':
		document.getElementById('colorinfo').children[0].innerHTML = "The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen.<br><br>A silver dun looks similar to a silver bay; however, a silver dun will have &quot;primitive&quot; markings.";
		break;
	case 'silver_dun_black':
		document.getElementById('colorinfo').children[0].innerHTML = "The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail, and is sometimes called chocolate flaxen.<br><br>The dun dilution gene and silver dapple gene work together to make the silver grulla truly a silver color.";
		break;
	case "silver_dun_champagne_sable":
	case "silver_dun_champagne_amber":
	case "silver_dun_champagne_classic":
		...
		break;
	case 'tovero':
		document.getElementById('colorinfo').children[0].innerHTML = "When pinto horse has both tobiano and overo markings, it is sometimes refered to as a tovero.  A Quarter Horse or Thoroughbred Pinto may also be called a Paint.  The flashy, spotted appearance of the Pinto is rare in the wild but has long been sought by human breeders.";
		break;
	case 'paint':
		document.getElementById('colorinfo').children[0].innerHTML = "A Quarter Horse or Thoroughbred Pinto may also be called a Paint.  The flashy, spotted appearance of the Pinto is rare in the wild but has long been sought by human breeders.";
		break;
	case 'tobiano':
		document.getElementById('colorinfo').children[0].innerHTML = "A tobiano pinto has large, white-haired, pink-skinned patches over its base coat.  The dark spots left behind are oval in shape and arranged in a vertical pattern.  A tobiano is usually white from the knees down, with a dark head and normal facial markings.<br><br>Tobiano is an allele of the KIT gene, making a homozygous tobiano mutually exclusive from roan, sabino, and dominant white.";
		break;
	case 'overo':
		document.getElementById('colorinfo').children[0].innerHTML = "An overo pinto has small irregular white spots over its base color, usually arranged in a horizontal pattern.  The lower legs tend to be dark and the head white or bald-faced.  The tail is a solid color, usually dark.<br><br>Overo has a connection to lethal white syndrome, as all LWS foals have overo in their pedigree.  However, the link is not directly connected to the expression of the color itself.  Many overos do not carry the lethal white gene.";
		break;
	case 'sabino':
		document.getElementById('colorinfo').children[0].innerHTML = "Sabino is characterized by irregular white spots on the extremeties, face, and belly.  The border edges of the spots often have a roaning appearance.<br><br>In its homozygous form, sabino results in a horse that is 90% or more white.  It is common for a homozygous sabino to be pure white and have pink skin, but without blue eyes.  The eye color can distinguish a sabino from cremellos or dominant white horses.<br><br>Sabino is an allele of the KIT gene, making a homozygous sabino mutually exclusive from roan, tobiano, and dominant white.";
		break;
	case 'splashed':
		document.getElementById('colorinfo').children[0].innerHTML = "Splashed white pintos are recorded as being in the overo family, but are not related to the overo gene.  It resembles a reverse tobiano, with white moving from the bottom of the horse towards the top.  Often the head will look as if dunked in a bucket of paint.<br><br>Splash is an incomplete dominant trait, and frequently heterozygous splash horses go unidentified as splash.";
		break;
	case 'roan':
		document.getElementById('colorinfo').children[0].innerHTML = "Roan horses have a mixture of white hairs in with their base color.  Unlike a grey horse, their color will remain consistent throughout life.<br><br>Roan is an allele of the KIT gene, making a homozygous roan mutually exclusive from tobiano, sabino, and dominant white.";
		break;
	case 'roan_red':
		document.getElementById('colorinfo').children[0].innerHTML = "Roan horses have a mixture of white hairs in with their base color.  Unlike a grey horse, their color will remain consistent throughout life.  When a chestnut horse shows roaning, it is referred to as a strawberry roan.";
		break;
	case 'roan_blue':
		document.getElementById('colorinfo').children[0].innerHTML = "Roan horses have a mixture of white hairs in with their base color.  Unlike a grey horse, their color will remain consistent throughout life.  When a black horse shows roaning, it is referred to as a blue roan.";
		break;
	case 'rabicano':
		document.getElementById('colorinfo').children[0].innerHTML = "Rabicano horses have white hairs intermibled with their base coat color in limited areas such as the belly, flanks, legs, and edges of the tail.";
		break;
	case 'sooty':
		document.getElementById('colorinfo').children[0].innerHTML = "Sooty horses have black hairs mixed in with their base coat color.  These are usually denser over the back, hips, and shoulders.";
		break;
	case 'appaloosa_varnish':
		document.getElementById('colorinfo').children[0].innerHTML = "Appaloosas have mottled skin, a white sclera, and stripped hooves.  Varnish roan appaloosas have an uneven roaning pattern which increases with age.  The speed of the roaning may vary from horse to horse.<br><br>Varnish roan genetics are required for any other appaloosa patterning to show.";
		break;
	case 'appaloosa_blanket':
		document.getElementById('colorinfo').children[0].innerHTML = "Appaloosas have mottled skin, a white sclera, and stripped hooves.  Blanket appaloosa horses have a distinctive spotting pattern that is usually limited to the back, hips, and rump.";
		break;
	case 'appaloosa_leopard':
		document.getElementById('colorinfo').children[0].innerHTML = "Appaloosas have mottled skin, a white sclera, and stripped hooves.  Leopard appaloosa horses have a distinctive leopard spotting pattern that covers most if not all of the horse.";
		break;
	case 'appaloosa_snowcap':
		document.getElementById('colorinfo').children[0].innerHTML = "Appaloosas have mottled skin, a white sclera, and stripped hooves.  Blanket appaloosa horses have a distinctive spotting pattern that is usually limited to the back, hips, and rump.<br><br>When a blanket appaloosa is homozygous for the varnish roan pattern, its spots no longer form and it is called a snowcap appaloosa.";
		break;
	case 'appaloosa_fewspot':
		document.getElementById('colorinfo').children[0].innerHTML = "Appaloosas have mottled skin, a white sclera, and stripped hooves.  Leopard appaloosa horses have a distinctive leopard spotting pattern that covers most if not all of the horse.<br><br>When a leopard appaloosa is homozygous for the varnish roan pattern, its spots no longer form and it is called a fewspot appaloosa.";
		break;
	case "pearl_chestnut":
		document.getElementById('colorinfo').children[0].innerHTML = "A homozygous pearl horse is diluted to a warm color with its points being diluted less than its body.<br><br>Pearl chestnuts are often called &quot;apricot.&quot;  The name was popularized before the discovery of the pearl allele.";
		break;
	case "pearl_bay":
	case "pearl_black":
	case "pearl_sealbrown":
		document.getElementById('colorinfo').children[0].innerHTML = "A homozygous pearl horse is diluted to a warm color with its points being diluted less than its body.";
		break;
	case "creampearl_chestnut":
	case "creampearl_black":
	case "creampearl_sealbrown":
	case "creampearl_bay":
		document.getElementById('colorinfo').children[0].innerHTML = "The pearl allele enhances a cream allele and causes the horse to look like a double dilute cream.  Thus a cream pearl is also sometimes called a pseudo double dilute.<br><br>Cream pearls are nearly impossible to differentiate from double dilute creams without genetic testing, but they have different breeding results.";
		break;
	case "silver_creampearl_black":
	case "silver_creampearl_sealbrown":
	case "silver_creampearl_bay":
		...
		break;
	case "dun_pearl_chestnut":
		document.getElementById('colorinfo').children[0].innerHTML = "A homozygous pearl horse is diluted to a warm color with its points being diluted less than its body.<br><br>Pearl chestnuts are often called &quot;apricot.&quot;  The name was popularized before the discovery of the pearl allele.<br><br>Duns are characterized by a lighter body color and &quot;primitive&quot; markings which include leg barring and a dorsal stripe.  Less common primitive markings incude ear barring, spider-webbed forehead, a lower face mask, and light &quot;guard hairs&quot; on the edges of the mane and tail.";
		break;
	case "dun_pearl_black":
	case "dun_pearl_sealbrown":
	case "dun_pearl_bay":
		document.getElementById('colorinfo').children[0].innerHTML = "A homozygous pearl horse is diluted to a warm color with its points being diluted less than its body.<br><br>Duns are characterized by a lighter body color and &quot;primitive&quot; markings which include leg barring and a dorsal stripe.  Less common primitive markings incude ear barring, spider-webbed forehead, a lower face mask, and light &quot;guard hairs&quot; on the edges of the mane and tail.";
		break;
	case "silver_dun_pearl_black":
	case "silver_dun_pearl_sealbrown":
	case "silver_dun_pearl_bay":
	...
	case "champagne_sable_pearl":
	case "champagne_classic_pearl":
		document.getElementById('colorinfo').children[0].innerHTML = "A homozygous pearl horse is diluted to a warm color with its points being diluted less than its body.<br><br>Champagne horses have pink, freckled skin and eyes often described as hazel.";
		break;
	case "pseudocream_gold":
	case "pseudocream_amber":
	case "pseudocream_sable":
	case "pseudocream_classic":
		document.getElementById('colorinfo').children[0].innerHTML = "When a horse recieves many different dilution genes it will become very light and difficult to distinguish.<br><br>A homozygous pearl horse is diluted to a warm color with its points being diluted less than its body.<br><br>Champagne horses have pink, freckled skin and eyes often described as hazel.<br><br>Duns are characterized by a lighter body color and &quot;primitive&quot; markings which include leg barring and a dorsal stripe.  Less common primitive markings incude ear barring, spider-webbed forehead, a lower face mask, and light &quot;guard hairs&quot; on the edges of the mane and tail.";
		break;
	case 'silver_champagne_classic_pearl':
	case 'silver_champagne_sable_pearl':
	case 'silver_champagne_amber_pearl':
		document.getElementById('colorinfo').children[0].innerHTML = "A homozygous pearl horse is diluted to a warm color with its points being diluted less than its body.<br><br>Champagne horses have pink, freckled skin and eyes often described as hazel.  When a seal brown horse receives a champagne gene, it will look like a cross between a classic champagne and an amber champagne.<br><br>The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail.";
		break;
	case "silver_pearl_bay":
	case "silver_pearl_black":
	case "silver_pearl_sealbrown":
		document.getElementById('colorinfo').children[0].innerHTML = "A homozygous pearl horse is diluted to a warm color with its points being diluted less than its body.<br><br>The silver dapple gene is the opposite of the cream gene: it affects only black hair.  It has a disproportionate effect on the mane and tail.";
		break;
	//default:
...
export function prepAnims() {
	...
	if (greyLayer.addEventListener) {
		greyLayer.addEventListener('webkitAnimationIteration',function(event){pauseAnim();},false);
		greyLayer.addEventListener('animationiteration',function(event){pauseAnim();},false);
	}
}
...
const GENES = ["e", "a", "g", "cr", "d", "ch", "f", "z", "sty", "rb", "rn", "sb", "w", "to", "o", "spl", "lp", "patn1", "patn2"];
...
async function crdtSetup() {
	...
	for (const gene of GENES) {
		for (const num of [1, 2]) {
			...
			alleles[alleleName] = runtime.registerCrdt(alleleName,
