...
Compoventuals is designed to let you create custom type-safe collaborative data models. By _data model_, we mean the model in a model-view-\* architecture: the part that holds the application state. A _collaborative data model_ is then the shared state in a collaborative app, which all users can edit, and which automatically propagates these edits to all users.

In addition to making data models for entire apps, you can make reusable data models for parts of an app. These serve a similar purpose to React Components, but for shared state instead of for the UI. You can even publish them as [Custom Types](TODO) for others to use.
...
2. Replace collection types (`Set`, etc.) and primitive types (`boolean`, etc.) with collaborative versions, following the advice in [Collaborative Data Types](TODO).
...
This completes our data model. To actually use this data model, we also have to integrate it with the view (Canvas), by updating the view in response to [Events](TODO) (either from the local user or other collaborators). For example:
...
- `flag: FlagStatus`: Recall that `FlagStatus` is a custom enum. As an opaque immutable type, the table in [Types](TODO) suggests `LwwCRegister<FlagStatus>`. In case of concurrent changes to the flag, this will pick one arbitrarily, which seems fine from the users' perspective.
...
Also, per step 3, we should replace `Tile` with a subclass of `CObject`. That leads to the class `CTile` below: (TODO: link to CObject guide/docs)
...
Finally, we need to convert the variable `currentGame: Minesweeper` that holds the app's top-level state. Since games can be created dynamically - there's not just a single game the whole time - this is really a _reference_ to a Minesweeper object. The table in [Types](TODO) suggests `LwwMutCRegister<CMinesweeper>` because the game is internally mutable:
...
This completes our data model. To actually use this data model, we also have to integrate it with the view, by updating the view in response to [Events](TODO) (either from the local user or other collaborators). An easy (though inefficient) way to do this is to refresh the entire view whenever anything changes:
