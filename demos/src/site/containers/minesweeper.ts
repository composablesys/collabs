...
const container: ContainerSource = {
  ...
  attachNewContainer(shadowDocument, runtime) {
    // HTML
    shadowDocument.innerHTML = `
      <style>
        .cell {
    ...
    // JS
    const board = shadowDocument.getElementById("board");
    const winText = shadowDocument.getElementById("winText")!;
    ...
    // Settings inputs
    let widthInput = shadowDocument.getElementById("width") as HTMLInputElement;
    let heightInput = shadowDocument.getElementById(
      "height"
    ) as HTMLInputElement;
    let percentMinesInput = shadowDocument.getElementById(
      "percentMines"
    ) as HTMLInputElement;
    ...
    shadowDocument.getElementById("newGame")!.onclick = function () {
    ...
    class TileCrdt extends crdts.CompositeCrdt {
      ...
      get status(): TileStatus {
        if (this.revealed.value) {
          return this.isMine
            ? TileStatus.REVEALED_MINE
            : TileStatus.REVEALED_EMPTY;
        } else {
      ...
      leftClick(x: number, y: number) {
        ...
        if (!tile.isRevealed()) {
          ...
          if (
            ...
            status === TileStatus.QUESTION_FLAG
          ) {
