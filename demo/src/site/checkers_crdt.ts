...
let redsPieces = document.querySelectorAll<HTMLElement>(".red-piece");
let blacksPieces = document.querySelectorAll<HTMLElement>(".black-piece")
const redTurnText = document.querySelectorAll<HTMLElement>(".red-turn-text");
const blackTurntext = document.querySelectorAll<HTMLElement>(".black-turn-text");
...
export class CheckersCrdt extends crdts.Crdt<number[]> {
    // player properties
    board: Array<any>;
    turn: number;
    redScore: number;
    ...
    constructor(id: any, runtime: network.CrdtRuntime) {
        super(
            id,
            new crdts.ArrayCrdtInternal(new crdts.CheckersInternal()),
            runtime,
            cboard
        );
        this.board = cboard
    ...
    private givePiecesEventListeners() {
        if (this.turn == 0) {
    ...
    private resetSelectedPieceProperties() {
    ...
    private checkAvailableJumpSpaces() {
        if (this.turn == 0) {
    ...
    private makeMove(number: number) {
    ...
    private changeData(indexOfBoardPiece: number, modifiedIndex: number, removePiece: number) {
