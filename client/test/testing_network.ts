...
import {CausalTimestamp, BroadcastNetwork, DefaultCausalBroadcastNetwork} from "../src/network";
...
export class TestingNetworkGenerator {
    newRuntime(replicaId?: string) {
        ...
        let runtime = new CrdtRuntime(new DefaultCausalBroadcastNetwork(replicaId, network));
    ...
    release(senderRuntime: CrdtRuntime, ...recipientRuntimes: CrdtRuntime[]) {
        ...
        let recipients = recipientRuntimes.map(runtime => this.getTestingNetwork(runtime));
    ...
    releaseAll() {
        for (let sender of this.messageQueues.keys()) this.release(sender.causal.crdtRuntime);
    }
