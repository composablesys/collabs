...
That is where **dynamically-created `Collab`s** come in. Collection types containing `Collab` elements, such as [DeletingMutCSet](../api/collabs/classes/DeletingMutCSet.html) (set), [DeletingMutCList](../api/collabs/classes/DeletingMutCList.html) (list), and [LazyMutCMap](../api/collabs/classes/LazyMutCMap.html) (map), let you create `Collab`s via their own `Collab` operations, like [DeletingMutCSet.add](../api/collabs/classes/DeletingMutCSet.html#add). We discuss those collections later in the Guide in [Built-In `Collab`s](./built_in_collabs.html#mutable-value-collections), but for now, we will explain how they initialize dynamically-created `Collab`s.
