...
class JSONTextTodoListCRDT implements TodoListCRDT {
  ...
  getItem(index: number): TodoListCRDT {
    return new JSONTextTodoListCRDT(
  ...
  implements TodoListReplica
{
  ...
  skipLoad() {
    this.app.load(
      collabs.Optional.of(JSONTextTodoList.getFakeInitialSaveData())
    );
  }

  load(saveData: Uint8Array): void {
    super.load(saveData);
    this.rootTodoList = new JSONTextTodoListCRDT(
