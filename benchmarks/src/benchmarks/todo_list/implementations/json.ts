...
class JSONTodoListCRDT implements TodoListCRDT {
...
export class JSONTodoList extends CollabsReplica implements TodoListReplica {
  ...
  skipLoad() {
    this.app.load(collabs.Optional.of(JSONTodoList.getFakeInitialSaveData()));
  }

  load(saveData: Uint8Array): void {
    super.load(saveData);
    this.rootTodoList = new JSONTodoListCRDT(this.rootJSON.value as JSONObject);
  }
